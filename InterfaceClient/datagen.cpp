...
void dataGenForAerocraft::generate(char *buf, int size)
{
	...
	unsigned int x = abs(int(m_x/0.001f));
	buf[4] = (x >> 16) & 0xFF;
	if(m_x < 0)
		buf[4] = buf[4] | 0x80;
	buf[5] = (x >> 8) & 0xFF;
	...
	unsigned int y = abs(int(m_y/0.001f));
	buf[7] = (y >> 16) & 0xFF;
	if(m_y < 0)
		buf[7] = buf[7] | 0x80;
	buf[8] = (y >> 8) & 0xFF;
	...
	unsigned int z = abs(int(m_z/0.001f));
	buf[10] = (z >> 16) & 0xFF;
	if(m_z < 0)
		buf[10] = buf[10] | 0x80;
	buf[11] = (z >> 8) & 0xFF;
	...
	unsigned int vx = abs(int(m_vx/0.0015f));
	buf[13] = (vx >> 16) & 0xFF;
	if(m_vx < 0)
		buf[13] = buf[13] | 0x80;
	buf[14] = (vx >> 8) & 0xFF;
	...
	unsigned int vy = abs(int(m_vy/0.0015f));
	buf[16] = (vy >> 16) & 0xFF;
	if(m_vy < 0)
		buf[16] = buf[16] | 0x80;
	buf[17] = (vy>> 8) & 0xFF;
	...
	unsigned int vz = abs(int(m_vz/0.0015f));
	buf[19] = (vz >> 16) & 0xFF;
	if(m_vz < 0)
		buf[19] = buf[19] | 0x80;
	buf[20] = (vz>> 8) & 0xFF;
	...
	unsigned int a1 = abs(int(m_angle1/precision));
	buf[22] = (a1>> 8) & 0xFF;
	if(m_angle1 < 0)
		buf[22] = buf[22] | 0x80;
	buf[23] = a1  & 0xFF;

	unsigned int a2 = abs(int(m_angle2/precision));
	buf[24] = (a2>> 8) & 0xFF;
	if(m_angle2 < 0)
		buf[24] = buf[24] | 0x80;
	buf[25] = a2  & 0xFF;

	unsigned int a3 = abs(int(m_angle3/precision));
	buf[26] = (a3>> 8) & 0xFF;
	if(m_angle3 < 0)
		buf[26] = buf[26] | 0x80;
	buf[27] = a3  & 0xFF;
	...
	unsigned int p1 = abs(int(m_p1/precision));
	buf[28] = (p1>> 8) & 0xFF;
	if(m_p1 < 0)
		buf[28] = buf[28] | 0x80;
	buf[29] = p1  & 0xFF;

	unsigned int p2 = abs(int(m_p2/precision));
	buf[30] = (p2>> 8) & 0xFF;
	if(m_p2 < 0)
		buf[30] = buf[30] | 0x80;
	buf[31] = p2  & 0xFF;

	unsigned int p3 = abs(int(m_p3/precision));
	buf[32] = (p3>> 8) & 0xFF;
	if(m_p3 < 0)
		buf[32] = buf[32] | 0x80;
	buf[33] = p3  & 0xFF;
